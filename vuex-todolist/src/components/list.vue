<template>
    <div>
       ...
       <ul>
           <li @click="finish(item)" v-for='(item,index) in list' :key="index" >{{item.text}} <span v-if="item.finish">√</span><span v-else>X</span> </li>
       </ul>
    </div>
</template>
...
<style scoped>
/* :style="{textDecoration:item.finish?'line-through':''}" */
   li>span{
       color:red;
   }
</style>